...
from dfnrec.orientation.fisher_mle import estimate_fisher_orientation
...
for sid in ["S1", "S2", "S3", "S4", "S5"]:
    ori = estimate_fisher_orientation(discs, sid, faces=faces)
    print(f"Set {sid}: n_used={ori.n_discs_used if ori else 0}, trend={ori.mean_trend_deg if ori else None:.2f}, plunge={ori.mean_plunge_deg if ori else None:.2f}, kappa={ori.kappa if ori else None:.2f}")
