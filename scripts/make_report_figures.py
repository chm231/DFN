...
실측 데이터(storage/output/pipeline_test_laxemar/)에 근거해 그린다.
  ① 절리 전처리         : rough face mesh+trace / 길이·중심·방향각 / censoring / 짧은 trace 제거
...
def polyline_yz(tr, i):
    s, c = tr["pl_start"][i], tr["pl_count"][i]
    v = tr["pl_xyz"][s:s + c]
    return v[:, 1], v[:, 2]


# --------------------------- 블록 ① 전처리 ---------------------------------
def block1(base, tr, out):
    poly = tr["tunnel_poly_yz"]
    fig, ax = plt.subplots(2, 2, figsize=(13, 11))

    # (a) rough face mesh + trace 추출
    mesh_path = os.path.join(base, "rough_face_mesh", "rough_face_mesh_collection.h5")
    axa = ax[0, 0]
    with h5py.File(mesh_path, "r") as f:
        gy = f["grid/grid_y"][:]
        gz = f["grid/grid_z"][:]
        rough = f["rough_faces/face_000001/field/rough_x"][:]
        mask = f["rough_faces/face_000001/field/inside_mask"][:].astype(bool)
    rough_m = np.ma.masked_where(~mask, rough)
    pc = axa.pcolormesh(gy, gz, rough_m, cmap="Greys", shading="auto", alpha=0.85)
    cb = fig.colorbar(pc, ax=axa, fraction=0.046, pad=0.04)
    cb.set_label("막장면 요철 오프셋 rough_x (m)")
    sel = np.where(tr["face_id"] == 1)[0]
    seen = set()
    for i in sel:
        y, z = polyline_yz(tr, i)
        s = int(tr["set_id"][i])
        lab = f"Set {s}" if s not in seen else None
        seen.add(s)
        axa.plot(y, z, "-", color=SET_COLORS.get(s, "0.3"), lw=1.1, label=lab)
    axa.plot(poly[:, 0], poly[:, 1], "k-", lw=1.5)
...
def main():
    ...
    for p in (block1(args.base, tr, args.out),
