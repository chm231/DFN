...
#   - --likelihood-mode {hybrid(기본·v1 최종)|window_mc(legacy)}: 우도 계산 방식.
#     hybrid = 참 현길이 분포를 닫힌형(해석식)으로, 창·절단 변환은 kr불변
#     MC 커널 1회로 분해. lmin_fit 미지정 시 전 set 공통 0.5 m 고정(§4.4).
#     ※ 종전 §4.6의 "window_mc 우위" 판정은 lmin 선택이 참값을 사용하는
#     오라클 평가였음이 확인되어(§4.7), 참값 미사용(blind·실측 동작) 재판정에서
#     hybrid 가 RMSE 3승 1무로 기본 채택됨. lmin 선택도 v1부터 참값 미사용
#     (best_row: fit상태→class_l1→q90) 으로 통일(D013 준수).
...
#   [v1 수정 — 참값 미사용(blind) 선택] 종전 구현은 정렬키에 |kr_hat - kr_true| 와
#   참값 기반 adoption(recovery 경유)을 사용해 벤치마크에서 오라클 선택으로 동작했고
#   (D013 위반), 참값이 없는 실측에서는 다른 규칙으로 퇴화하는 불일치가 있었다.
#   v1 부터는 벤치마크·실측 동일하게 **데이터 기반 진단만으로** 선정한다:
#     (적합상태 ok 우선 -> rmin 지지 불일치 최후순위 -> 클래스분율 L1오차 -> q90비 근접도)
#   kr_true / kr_abs_error 는 요약 CSV 의 사후 검증용 컬럼으로만 남는다.
#   반환: 정렬상 최상위 행
def best_row(rows: Sequence[dict]) -> dict:
    ...
    # 행의 정렬 키(튜플)를 구성. 앞쪽 요소일수록 우선순위가 높다 (참값 미사용)
    def sort_key(row: dict) -> Tuple[float, float, float]:
        fit = str(row.get("fit_status", ""))
        adoption = str(row.get("adoption_status", "rejected"))
        tier = 0 if fit in ("ok", "provisional_ok") else 1
        if adoption == "diagnostic_only_rmin_support_mismatch":
            tier = 2
        return (
            float(tier),
...
def main() -> None:
    ...
    parser.add_argument("--likelihood-mode", choices=["window_mc", "hybrid"], default="hybrid",
                        help="hybrid(기본·v1)=참 현길이 분포는 닫힌형(해석식), 창·절단 변환만 "
                             "kr불변 MC 커널로 1회 계산. 참값 미사용(blind) 조건 20시드 재판정에서 "
                             "RMSE 우위/동률(3승 1무)로 기본 채택. window_mc=legacy 전량 MC 시뮬레이션.")
