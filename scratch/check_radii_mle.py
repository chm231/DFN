...
print("Pareto MLE fit on reconstructed discs:")
for sid in ["S1", "S2", "S3", "S4", "S5"]:
    set_discs = [d for d in discs if d.set_id == sid]
    radii = np.array([d.radius_m for d in set_discs])
    if len(radii) >= 2:
        r_min = np.min(radii)
        alpha = len(radii) / np.sum(np.log(radii / r_min))
        k_r = alpha - 1.0
        print(f"Set {sid}: count={len(radii)}, r_min={r_min:.4f}, alpha={alpha:.4f}, k_r={k_r:.4f}")
    else:
        print(f"Set {sid}: count={len(radii)}, insufficient data")
