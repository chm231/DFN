...
from .slab_utils import extract_slab_points_from_truth, extract_slab_segments_from_truth
...
def main():
    ...
    print(f"\n[2/6] Extracting clusters and local planes within each Slab (3D Segment Based)...")
    all_candidates = {}
    all_slab_segments = [] # CAD 내보내기용
    
    for slab in tqdm(slabs, desc="Processing Slabs"):
        # 3D 선분(Traces) 직접 추출
        slab_traces = extract_slab_segments_from_truth(centers, normals, radii, slab, poly_yz, sub_slice_count=args.subslices)
        
        # CAD 시각화를 위해 선분 형태로 리스트 유지
        ...
        clusters_idx_list = cluster_3d_segments(
            slab_traces, 
            dist_threshold=args.eps,            # 기존 eps 인자를 거리 임계값으로 매핑
            angle_penalty_threshold=0.8,        # 코사인 유사도 기준 각도 임계값
