...
각 (방위각 theta, realization seed) 마다 파이프라인:
    ...
    -> trace 기반 config(trend/plunge/kappa/kr_hat) -> P32(analytic_esinphi)
...
def _process_one(py, env, sets, label, args, a, sd, wd, base_h5, report=None):
    ...
    _run([py, "-m", "dfn_analysis.estimate_p32_mc_calibrated",
          ...
          # 2026-08-04 재확정: 기본·최종 = 해석식 C=E[sinφ] (P21 전량, lmin 미적용)
          "--calibration-factor-mode", "analytic_esinphi", "--lmin", str(args.lmin),
