...
#   2) 각 (set, lmin_fit)에 대해 kr 격자를 순회하며 모델 확률표를 만든다.
#      우도 방식은 likelihood_mode 로 선택:
#      (a) window_mc (기본, 기존 방식) — kr 격자점마다 전량 MC 시뮬레이션:
#          kr로 size-biased 반경 표본 → 참(true) 현(chord) 길이 표본
#          → 방향(경험적/방위조건부) 표본 → 관측창 클리핑으로 가시길이/검열등급
#          → 길이×등급 가중 히스토그램 → 확률표 p_ic(kr)
#      (b) hybrid (신규) — 우도를 두 인자로 분해해 kr 의존부를 해석식으로 평가:
#            p(ℓ_vis, c | kr) ∝ Σ_j w_j(kr) · K[j, i, c]
#          · w_j(kr) = 참 현길이 bin 질량. 크기편향 절단멱법칙 g_R∝r^-kr 을
#            주변화한 닫힌형 적분(analytic_true_chord_bin_masses) → MC 잡음 없음.
#          · K[j,i,c] = 창·절단 변환 커널(참 현길이 bin j → 가시길이 bin i,
#            검열등급 c). kr 과 무관하므로 kr 격자 밖에서 1회만 MC 계산
#            (HYBRID_TRUE_BINS=64 로그빈 × HYBRID_KERNEL_SAMPLES_PER_BIN=3000).
#          · 진단/사후예측 표본은 최적 kr 에서 고전 시뮬레이션 1회로 보충.
#          효과: 프로파일 우도 재표집 잡음 제거(2차차분 RMS 수십배↓),
#                lmin 후보 간 kr_hat 안정화, 조합당 계산량 감소.
#      두 방식 모두 관측 히스토그램과의 다항 로그우도로 kr 을 평가한다.
