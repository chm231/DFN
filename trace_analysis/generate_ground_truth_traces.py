...
def main():
    ...
    print(f"  -> GT DFN fracture count: {len(gt_radii):,}")
    ...
    print(f"  -> Created {len(faces)} excavation faces at x = {list(x_positions)} m")
    ...
    if not args.flat:
        print(
            f"[*] Generating rough faces and extracting traces "
            f"(amplitude={args.dx}m, lc={args.lc}m, res={args.res}m)"
        )
        fracture_data = {
            "centers": gt_centers,
            "normals": gt_normals,
        ...
        print("[*] Using flat faces for ground-truth trace generation")
        for face in faces:
    ...
    print(f"  -> Extracted {len(obs_traces)} trace segments")

    # 3. Export to HDF5
    h5_path = args.output_prefix + ".h5"
    print(f"[*] Writing HDF5: {h5_path}")
    ...
    parent_ids = np.array(
        [t.parent_fracture_id if t.parent_fracture_id is not None else -1 for t in obs_traces],
        dtype=np.int32,
    )
    ...
    print("\n[Done] Ground truth trace datasets created successfully.")
