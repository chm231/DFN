...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="analytic_esinphi(기본·최종, 2026-08-04 재확정)=수식 기반 C=E[sinφ] 결정론적 "
             "구적. forward_mc_lmin=C_lmin=E[sinφ]·η_det 를 순방향 모사에서 직접 산정"
             "(관측·가상에 동일 lmin 적용). unit_p32_forward_mc=legacy 순방향 MC"
             "(면적 기준 불일치로 C +2~4%% 과대), proxy=스캐폴드.",
    )
