...
# =============================================================================
# 파일 역할:
#   원판 복원 검증 시각화. reconstruct 가 만든 복원 disc 와 관측 trace 를 겹쳐,
#   복원이 관측을 재현하는지(빨강 chord 가 파랑 trace 를 덮는지) 눈으로 확인한다.
#   시각화 전용 — 기하 계산/변경 없음(CLAUDE.md §12).
#
# 주요 입력:
#   - --recon-csv : 복원 disc CSV(reconstructed_discs.csv)
#   - --trace-h5  : 관측 trace HDF5(/traces 끝점·set_id·face_x, /meta 폴리곤·면 x위치)
#   - --outdir    : 그림 저장 폴더
#   - --sets / --multi-face-only / --max-discs : 표시 disc 필터(특정 set / 다면관통 / 표본수)
#
# 주요 출력:
#   - reconstruction_3d.png         : 3D 개요(막장면 윤곽 + 관측 trace + 복원 원)
#   - reconstruction_face_x{xf}.png : 면별 2D 오버레이(관측 trace vs 복원 원판 chord)
#
# 핵심 처리 흐름:
#   1) 복원 disc·관측 trace·터널 폴리곤/면 x위치 로드(--sets 등 필터 적용)
#   2) 3D 개요: 막장면 윤곽 + 관측 trace(파랑) + 복원 원(반지름 상태별 색) 렌더 → PNG
#   3) 면마다 복원 disc 의 window-clip chord(빨강) vs 관측 trace(파랑) 오버레이,
#      미재현 관측 강조 → 면별 PNG
# =============================================================================
import argparse
import csv
