...
%% =========================================================
%  - full visualization oriented
%  - keep only discs fully inside the cube
%% =========================================================

outDir = 'dfn_output_cube10m';
if ~exist(outDir, 'dir')
    mkdir(outDir);
end
...
%% -----------------------------
%  DFN SETS
%  For small cube visualization, rmax should be reduced
%% -----------------------------
sets = [];

rmin_explicit = 0.40;   % or 0.40 for sparser case
rmax_model    = 8.0;    % strongly recommended for 10 m cube
...
for s = 1:numel(sets)
    ...
    metadata.box          = box;
    metadata.note         = 'Discs are generated in the cube and boundary-intersecting discs are clipped only at plotting stage.';
...
% full visualization
plot_clipped_dfn(masterFile);
%plot_all_sets_full_cube(masterFile, true)
%plot_all_sets_full_cube(masterFile, true); 원판 전체를 그대로 그림 or
%plot_clipped_dfn(masterFile); clipping 후